...
export default function SegUsuarios() {
      ...
      },
      {
        ...
        render: (row) => (
          <div className="flex justify-center gap-2">
            <button
              className="px-2 py-1 text-xs rounded-md bg-[#0b1a38] text-white hover:bg-[#1a5d42]"
              onClick={() => {
                setEdit(row);
                setOpen(true);
              }}
  ...
  return (
    ...
    >
      ...
      <div className="bg-white rounded-2xl border border-[#dce8fd] p-5 md:p-6 mb-5">
        ...
        <div>
          ...
          <div className="hidden md:block">
            <DataTable
              ...
              headerClass="px-4 py-3 font-semibold border-r border-[#dce8fd] last:border-none select-none"
              cellClass="px-4 py-3 border-r border-[#e2e8f0] last:border-none"
            />
          </div>
            ...
            {filtered.map((u) => (
              <div key={u.id_usuario} className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h3 className="font-bold text-[#0b1a38] text-lg">{u.nombre}</h3>
                    <p className="text-sm text-gray-600">{u.mail}</p>
                  </div>
                  <span className={`px-2 py-1 rounded text-xs font-semibold ${u.estado === "ACTIVO" ? "bg-[#f0f5ff] text-[#102354]" : "bg-slate-100 text-slate-600"
                    }`}>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 bg-[#0b1a38] text-white py-2 rounded-lg text-sm hover:bg-[#102354]"
                  >
                    Modificar
                  </button>
                  <button
